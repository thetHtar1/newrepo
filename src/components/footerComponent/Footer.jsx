...
export default function FooterComponent() {
 ...
 const handleEmail=(e)=>{
  ...
  SetemailSubscribe(getEmail)
 }

 const handleSubmit =(e)=>{
  e.preventDefault();

  console.log(`get email ${emailSubscribe}`)
 }
  return (
    <>
   <Curve/>
   <main className='bg-black px-10 md:px-20'>
    ...
    <div className='mt-20 sm:mt-40 font-inter flex flex-col justify-start gap-11  sm:flex-row sm:justify-between sm:items-end'>
      ...
      <div className=''>
        ...
        <div className='flex flex-col sm:flex-row relative pt-2 ' onSubmit={handleSubmit}>
          <div className=''>
            <input className='bg-transparent text-white border-b-2 lg:pr-20 sm:border-r border-r-0 sm:w-10/12 xs:w-full' type="text" placeholder='Your Email Please' onChange={(e)=>handleEmail(e)}/>
          </div>
          <div>
            <button className='border ml-2 rounded-full	px-2 py-1 absolute right-2  text-white hover:bg-white hover:text-black duration-500 mt-3 sm:-right-16 sm:-top-3'>Subscribe</button>
          </div>
        </div>
